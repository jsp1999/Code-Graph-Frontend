...
export const ClusterGraph: React.FC<ClusterProps> = ({
  ...
  const arrows = Object.entries(node_data)
    .map(([id, entry]) => ({
      id: parseInt(id),
  ...
  const all_nodes = node_data
    .map((d) => Object.create(d))
    .map(({ id, segment, sentence, x, y, annotation, cluster }) => {
  ...
  React.useEffect(() => {
    ...
    const svg = d3.select(svgChart.current);
    const circles = svg.selectAll("circle").attr("r", radius);
  ...
  React.useEffect(() => {
    ...
    function zoomed(event) {
      ...
      svg.attr("transform", transform).append("g");
